 ...
 * It handles logic for creating, deleting, and modifying different types of decks, ensuring
 * rules like capacity and card uniqueness are followed. It leverages polymorphism to
 * handle different deck types seamlessly.
 */
public class DeckManager {
    /**
     * The list of all decks managed by the system. It holds the abstract Deck superclass
     * to allow for polymorphic behavior between Normal and Sellable decks.
     */
    private final ArrayList<Deck> decks;

    /**
     * A reference to the central CollectionManager, required for updating card counts
     * when cards are moved or decks are deleted/sold.
     */
    private final CollectionManager collectionManager;

    /**
     * Constructs a new DeckManager.
     * @param collectionManager The central CollectionManager that this manager will depend on
     *                          for all card inventory operations.
     */
     ...
     * This method acts as a factory, instantiating the correct Deck subclass.
     * Fails if a deck with the same name already exists.
     * @param name The name for the new deck.
     * @param type The string representing the type of deck to create ("Normal" or "Sellable").
    ...
    public boolean deleteDeck(String name) {
        ...
        // Return all cards to the main collection
    ...
    /**
     * Sells a deck if it is sellable. The sale price is the sum of the real values
     * of all cards inside. When sold, the deck and its cards are removed permanently.
     * @param name The name of the deck to sell.
     * @return The calculated sale price if sold successfully. Returns a value <= 0 on failure.
     */
    public double sellDeck(String name) {
        ...
        // Polymorphism: The deck's own isSellable() method is called.
        if (!deckToSell.isSellable()) {
            System.out.println("Error: This deck type ('" + deckToSell.getClass().getSimpleName() + "') cannot be sold.");
            return 0.0;
        }
        ...
        // On a successful sale, the deck and its cards are removed permanently.
    ...
    public int addCardToDeck(String cardName, String deckName) {
        ...
        // The Deck's addCard method already checks for fullness and uniqueness.
        if (!deck.addCard(card)) {
            if (deck.isFull()) {
                return 3; // Failure because it's full.
            } else {
                return 4; // Failure because the card is a duplicate.
            }
        }
