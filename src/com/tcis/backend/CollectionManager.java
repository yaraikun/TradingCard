 ...
 * handling the core logic of card creation and inventory counts. All card name lookups
 * are case-insensitive.
 */
public class CollectionManager {
    /**
     * Stores a list of the unique Card objects that have been created. This list
     * defines all possible cards in the system.
     */
    private final ArrayList<Card> cardTypes;

    /**
     * Stores the quantity of each card. The key is the card's name in lowercase
     * to ensure case-insensitive lookups, and the value is the integer count.
     */
     ...
     * It relies on the Card constructor to throw an exception for invalid data, which is caught here.
    ...
    public boolean addNewCard(String name, double baseValue, Rarity rarity, Variant variant) {
        if (findCard(name) != null) {
            // This check is primarily for the UI to give a specific error message.
            return false;
        }
        try {
            ...
            // Catches errors from the Card constructor (e.g., negative value, blank name)
     ...
     * This is a specialized convenience method that calls {@code decreaseCount(cardName, 1)}.
     * @param cardName The name of the card to sell.
     * @return true if one copy of the card was successfully removed, false otherwise.
     */
     ...
     * @return A new HashMap containing card names (lowercase) and their quantities.
     */
